...
def clean_tennis_data(tennis_df_in, origin_date, name_to_id_dict):
    ...
    tennis_df['TimestampDays'] = (tennis_df['Timestamp'] - origin_date).dt.days.astype(int)
    tennis_df = tennis_df.sort_values('TimestampDays').reset_index(drop=True)
...
def load_wta_numpy(start_date: str = '2018-12-31', end_date: str = '2023-01-01',
             origin_date: str = '2018-12-31') -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict, dict]:
    data_2019 = pd.read_excel('data/2019.xlsx')
    data_2020 = pd.read_excel('data/2020.xlsx')
    data_2021 = pd.read_excel('data/2021.xlsx')
    data_2022 = pd.read_excel('data/2022.xlsx')
    data_all = pd.concat([data_2019, data_2020, data_2021, data_2022])
    players_arr = pd.unique(pd.concat([
        consolidate_name_strings(data_all['Winner']),
    ...
    match_times = np.array(data_all['TimestampDays'])
    match_player_indices = np.array(data_all[['WinnerID', 'LoserID']])
    match_results = np.ones_like(match_times)
